...
4. Đọc [data-migrations.md](references/data-migrations.md) cho dữ liệu riêng và [frontend-hooks-shortcodes.md](references/frontend-hooks-shortcodes.md) cho frontend, hook hoặc shortcode. Nếu plugin có bất kỳ giao diện nào, bắt buộc đọc [localization.md](references/localization.md).
...
- Plugin có giao diện frontend hoặc admin phải tự chứa ít nhất hai locale `vi` và `en`; `vi` là mặc định và fallback. Không đặt chuỗi nghiệp vụ của plugin trong bộ dịch theme. Chỉ bổ sung locale khác khi theme/yêu cầu cụ thể cần đến.
- Giữ locale qua GET, POST, validation và redirect; không chấp nhận giao diện chỉ dịch menu hoặc tiêu đề trong khi nội dung còn hard-code.

...
Use the actual variable (`$products`, `$posts`, `$testimonials`). Never call plain `$items->links()`.
...
Product/news controllers already call `withQueryString()`. Preserve existing search/category/sort keys in forms/links. Do not hand-build pagination query strings in Blade.

## Test

- Ensure data/settings produce at least two pages.
- Test page 1 and `?page=2` for products, category-filtered products, news, and testimonials.

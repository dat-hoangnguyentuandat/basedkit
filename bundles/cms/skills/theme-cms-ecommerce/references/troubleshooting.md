...
Cause: a required view is missing and Blade falls through to shared resources. Ship every route view in `views/`, especially `products/index`, `product`, `news/index`, `news/show`, `contact`, and the commerce plugin cart/checkout views. Clear compiled views.

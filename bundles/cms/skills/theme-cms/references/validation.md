...
Request every route the theme ships and representative detail routes. Require 200 for valid pages and intentional 404 for invalid slugs. Include `/`, `/gioi-thieu`, `/dich-vu`, `/dich-vu/{slug}`, `/san-pham`, `/san-pham/{slug}`, `/tin-tuc`, `/tin-tuc/{slug}`, `/lien-he`, and `/danh-gia`.
...
Use Playwright or an equivalent browser at 1440, 1024, 768, 390, 375, and 320px. Capture home, about, one listing, one detail, contact, reviews, and pagination page 2. Click representative hero, category, view-all, review, footer, and mobile-menu links and require the expected non-404 destination. Check no horizontal overflow, clipped text, giant SVG, long portrait image, broken logo, overlapping footer, or accidental old palette.
...
```powershell
php artisan tinker --execute="echo 'Products: '.App\Models\Product::count().', Services: '.App\Models\Service::count().', News: '.App\Models\News::count().', Testimonials: '.App\Models\Testimonial::count();"

...
| `services.index` | `/dich-vu` | `services.index` |
| `service.show` | `/dich-vu/{slug}` | `service` |
...
Do not link `/uu-dai`, `/bang-gia`, `/dat-lich`, or a standalone search route unless core actually defines it. Product/service search uses `?q=`; category filters use controller-supported query keys.
...
- About/hero: `about_title`, `about_desc`, `hero_heading`, `hero_lead`, `hero_desc`
- Pagination: `products_per_page`, `services_per_page`, `news_per_page`, `testimonials_per_page`, `search_per_page`, and related-count keys
...
- Home receives active products, services, categories, testimonials, FAQs, news, and grouped homepage blocks.
- Product/service listings support search, category filters, configured page size, and `withQueryString()`.
...
Seed six primary items when applicable: Trang chủ, Giới thiệu, Dịch vụ or Sản phẩm, Tin tức, Liên hệ, Đánh giá. Use `type`, `url`, `slug`, and optional `children`; never use nonexistent `route_name` column. Menu seed runs only on an empty table, so inspect existing menu data during testing.
...
Use `cms_content($value, $model)` for product, service, news, about, and other admin-authored rich content so core/plugin shortcodes and content hooks execute. Escape plain labels and settings with `{{ }}`.

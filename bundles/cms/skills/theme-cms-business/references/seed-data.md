...
Use `category_slug` in product/service/news seed rows; the seeder resolves IDs. Use unique slugs and valid model fields. Use `role`, not `position`, for testimonials.
...
Seed site identity, contact details, social links, pagination, about/hero content, Google review URL, and other core-supported settings. Use the exact keys documented in `cms-data-routes.md`. Do not seed stale logo paths as the authoritative header logo; admin `site_logo` remains the source of truth.

## Menu

Provide six valid main items where applicable. `menu_items` has no `route_name` column. Each item may use `label`, `type`, `url`, `slug`, `target`, ordering, SEO fields, and `children`.
...
- Services/products: enough cards for every homepage/list state requested.
...
Use paths such as `theme-assets/{slug}/images/service-a.jpg` without a leading slash. Confirm each source file exists under `themes/{slug}/assets` and is published to `public/theme-assets/{slug}`.

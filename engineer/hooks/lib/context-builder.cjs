...
const os=require('os');
const {loadConfig,readSessionState}=require('./basedkit-config-utils.cjs');
function stampPath(transcriptPath=''){const key=Buffer.from(transcriptPath||'default').toString('hex').slice(0,32);return path.join(os.tmpdir(),`basedkit-reminder-${key}.json`)}

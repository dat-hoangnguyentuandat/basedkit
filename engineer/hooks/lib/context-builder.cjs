...
const os=require('os');
const {loadConfig,readSessionState}=require('./bk-config-utils.cjs');
function stampPath(transcriptPath=''){const key=Buffer.from(transcriptPath||'default').toString('hex').slice(0,32);return path.join(os.tmpdir(),`bk-reminder-${key}.json`)}

/**
 * Shared utilities for BasedKit hooks
 *
...
const LOCAL_CONFIG_PATH = '.claude/.basedkit.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.basedkit.json');
...
function getSessionTempPath(sessionId) {
  return path.join(os.tmpdir(), `basedkit-session-${sessionId}.json`);
}
 ...
 * @param {Object} config - BasedKit config
 ...
 *   2. Global config (~/.claude/.basedkit.json) - user preferences
 *   3. Local config (./.claude/.basedkit.json) - project-specific overrides
 *
...
function resolveNamingPattern(planConfig, gitBranch) {
  ...
  if (!validation.valid) {
    ...
    if (process.env.BASEDKIT_DEBUG) {
      console.error(`[basedkit-config] Warning: ${validation.error}`);
    }
  }

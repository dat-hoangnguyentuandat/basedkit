...
**Tools and Commands:**
- **App Debug Skill**: Activate `app-debug` skill when debugging app issues — conversations, API logs, bot config. Contains query flow, table schema, environment mapping.
- **Query Scripts** (read-only, MySQL via SSH tunnel):
  - Production: `bash .claude/scripts/db-prod-query.sh "SQL_QUERY"`
  - Staging: `bash .claude/scripts/db-staging-query.sh "SQL_QUERY"`
  - Local: `bash .claude/scripts/db-local-query.sh "SQL_QUERY"`

 ...
 *
 * The session temp file (/tmp/basedkit-session-{id}.json) is the source of truth
 * for plan context within a session. Env vars ($BASEDKIT_ACTIVE_PLAN) are just
 * the initial snapshot from session start.
 */

const path = require('path');
const { writeSessionState, readSessionState } = require('../hooks/lib/basedkit-config-utils.cjs');

const sessionId = process.env.BASEDKIT_SESSION_ID;
...
if (!sessionId) {
  console.warn('Warning: BASEDKIT_SESSION_ID not set - session state will not persist');

...
# SEO: Universal SEO Analysis Skill — Manual First

> **Execution policy for this installation:** Perform SEO audits manually by default. Do not require `claude-seo`, Python runtimes, Chromium, Puppeteer, MCP services, API credentials, or automatic sub-agent orchestration. The modules below remain checklists and analysis references; their automated commands are optional enhancements only when the user explicitly requests automation and the runtime is already available.

## Manual Audit Contract

For every audit, inspect the rendered public URL with ordinary HTTP/browser tools available in the environment and report evidence. Never claim an automated module ran unless its command actually completed successfully.

1. Fetch the URL and required routes with `Invoke-WebRequest`, `curl`, a browser, or an equivalent read-only HTTP client.
2. Inspect the returned HTML for status code, title, meta description, language, viewport, canonical, robots directives, Open Graph tags, JSON-LD, H1 count, headings, internal links, forms, images, scripts, stylesheets and iframes.
3. Request every important asset URL and record status, content type, dimensions/weight where available, and missing alt text.
4. Check `/robots.txt` and `/sitemap.xml` manually. Treat a missing sitemap as an infrastructure finding, not as a reason to invent a crawl result.
5. Check representative listing/detail/contact pages, pagination and query-string filters. Record HTTP status and rendered page title for each.
6. Inspect responsive CSS and, when a browser is available, capture screenshots at 1440, 1024, 768, 390, 375 and 320px. If no browser runtime exists, say so and perform a source-level responsive checklist instead.
7. Validate local-business identity manually: business name, type, telephone, address, coordinates, map URL, rating and review count must agree across Settings, visible HTML and JSON-LD.
8. Classify findings as Critical, High, Medium or Low, include the exact URL/evidence, and separate verified findings from assumptions and unavailable checks.

### Manual command examples

```powershell
$res = Invoke-WebRequest -Uri 'http://localhost/cms/public/' -UseBasicParsing
$res.StatusCode
$res.Content | Select-String '<title>|<h1|canonical|application/ld\+json'

rg -n "->links\(\)|filemtime\(|noindex|canonical|application/ld\+json" themes\{slug} resources
```

For Laravel themes, also run `php artisan view:cache`, `php artisan view:clear`, the theme smoke routes, asset publish/version checks, and `php artisan theme:package {slug}` when packaging is in scope.

**Invocation:** `/seo $1 $2` where `$1` is the command and `$2` is the URL or argument. In this installation, commands resolve to the manual checklist first.

**Runtime:** No dedicated runtime is assumed. Use the tools available in the current environment. `claude-seo` commands, Python scripts, Chromium, Puppeteer, Lighthouse, MCP and external APIs are optional and must not be required for a manual audit.

Comprehensive SEO analysis across all industries (SaaS, local services,
e-commerce, publishers, agencies). It provides 24 manual checklists (21 core + 1
framework integration + 2 extension mirrors). Optional automation extensions are
documented for environments that explicitly provide them.

## Quick Reference

| Command | What it does |
|---------|-------------|
| `/seo audit <url>` | Full website audit using the manual checklist |
...
| `/seo setup` | Report optional automation availability; do not install anything by default |
| `/seo doctor` | Check optional runtime availability without changing the system |

## Runtime Setup

No setup is needed for the manual workflow. If an optional automation runtime is
missing, continue with the manual contract above and report the unavailable
capability. Do not install packages, launch Chromium, configure API credentials,
or modify the host environment unless the user explicitly asks for that setup.

## Orchestration Logic — Manual

When the user invokes `/seo audit`, inspect the site sequentially with the manual
contract. Do not automatically delegate to subagents. Use the following order:

1. Identify the business/page type from visible signals.
2. Fetch the homepage and representative routes.
3. Run technical and indexability checks: status, redirects, robots, sitemap, canonical, noindex, URL structure and mobile metadata.
4. Run on-page/content checks: titles, descriptions, H1–H3 hierarchy, copy depth, internal links and conversion paths.
5. Run schema/local checks: parse visible JSON-LD and compare NAP, geo, map, rating and review data.
6. Run image/asset checks: alt text, dimensions, content type, weight and broken URLs.
7. Run manual responsive/visual checks when a browser is available; otherwise document the missing browser runtime and inspect CSS breakpoints/source.
8. Run pagination, form and representative detail-page checks.
9. Synthesize findings with the 10-principle framework below and classify severity.
10. Produce a unified report and prioritized action plan. Optional automated modules may be mentioned as unavailable; do not fabricate their output.
...
## Module Responsibilities (No Automatic Subagents)

Use these responsibilities as sections of the manual report. Do not spawn or
...
- `seo-local` -- GBP signals, NAP consistency, reviews, local schema, industry-specific local factors (apply manually when Local Service detected)
- `seo-maps` -- Geo-grid rank tracking, GBP audit, review intelligence, competitor radius mapping (apply manually when Local Service detected AND DataForSEO MCP available)
- `seo-google` -- CWV field data, URL indexation status, organic traffic trends (apply manually when Google API credentials detected)
- `seo-backlinks` -- Backlink profile data: DA/PA, referring domains, anchor text, toxic links (apply manually when Moz/Bing API keys detected or always for CC domain-level metrics)
- `seo-cluster` -- Semantic clustering analysis (conditional: content strategy detected)
- `seo-sxo` -- Page-type mismatch, user stories, persona scoring (always check manually in full audits)
...
- `seo-flow` -- FLOW framework prompts (apply manually for content strategy workflows)
...
| Optional automation is unavailable | Continue with the manual checklist, record the unavailable capability and do not fabricate automated results. |
| A manual check cannot be performed | Mark the finding as unavailable, explain why, and provide the next safe verification step. |
| Ambiguous business type detection | Present the top two detected types with supporting signals. Ask the user to confirm before proceeding with industry-specific recommendations. |

---

# Integrated SEO Modules — Manual Checklists

This file consolidates the SEO core modules below into one skill. Use the relevant module section as a manual checklist. Automated commands shown later are optional references only; replace them with the manual HTTP/HTML/source checks defined at the top of this file when the optional runtime is unavailable.
...
1. **Fetch homepage manually**: use `Invoke-WebRequest`, `curl`, or an available browser to capture returned HTML and status code; record whether JavaScript is needed for critical content.
2. **Detect business type**: analyze homepage signals per seo orchestrator
3. **Crawl site manually**: follow a representative set of same-origin internal links, respect robots.txt, and state the scope instead of claiming a full crawl.
4. **Apply the module checklists inline** (do not automatically delegate to subagents):
   ...
   - `seo-geo` -- AI crawler access, llms.txt, citability, brand mention signals
   - `seo-local` -- GBP signals, NAP consistency, reviews, local schema, industry-specific local factors (apply when Local Service signals are visible)
   - `seo-maps` -- Geo-grid rank tracking, GBP audit, review intelligence, competitor radius mapping (use only when user-provided map/rank data is available; otherwise record unavailable)
   - `seo-google` -- CWV field data (CrUX), URL indexation (GSC), organic traffic (GA4) (use only when user-provided first-party exports are available; otherwise record unavailable)
   - `seo-backlinks` -- Backlink profile data: DA/PA, referring domains, anchor text, toxic links (use only when verified backlink data is available; otherwise record unavailable)
   - `seo-cluster` -- Semantic clustering analysis (apply when visible content-strategy signals exist)
   - `seo-sxo` -- Search experience analysis: page-type mismatch, user stories, persona scoring (always include in full audits)
   - `seo-drift` -- Drift analysis: compare against stored baseline (compare only against a supplied or locally stored baseline)
   - `seo-ecommerce` -- Product schema, marketplace intelligence (apply when e-commerce signals are visible)
